...
class Imap {
	...
	public function imap_connect($config = [])
	{
		...
		{
			$msg=$this->last_error();
			Mdebug_log::AddGeneralLog("Email to ticket imap config error", Mdebug_log::STATUS_FAILED, Mdebug_log::ENTRY_TYPE_ERROR, $msg);
			return $msg;
		}
    ...
    public function imap_connect_default(){
        $is_enable=Mapp_setting::GetSettingsValue("is_imap_ticket","N")=="Y";
        if($is_enable){
            $host=Mapp_setting::GetSettingsValue("imap_host","");
            $host=trim($host);
            $port=Mapp_setting::GetSettingsValue("imap_port","");
            $port=trim($port);
            $encyptoType=Mapp_setting::GetSettingsValue("imap_secure_type","ssl");
            $encrypto=Mapp_setting::GetSettingsValue("imap_is_secure")=="Y"?$encyptoType:NULL;
            
            $user=Mapp_setting::GetSettingsValue("imap_user");
            $pass=Mapp_setting::GetSettingsValue("imap_pass");
            $config = array(
                'host'     => "{$host}:{$port}",
                'encrypto' => $encrypto,
                'user'     => trim($user),
